...
download_daymet_tile <- function (tile_id, year, param, daymet_dir, force = FALSE, timeout = 30) {
  log_debug("downloading daymet tile (id={tile_id}, year={year}, param={param})")
  ...
  daymetr::nc2tif(path = daymet_dir, files = nc_filepath, overwrite = TRUE, silent = TRUE)
  
  if (!file.exists(tif_filepath)) {
    log_error("failed to convert nc file ({nc_filename}) to tif ({tif_filename})")
    return(NA_character_)
  }
...
collect_daymet_tile_files <- function (tile_years, daymet_dir = "data/daymet", params = c("tmin", "tmax")) {
  possibly_download_daymet_tile <- possibly(download_daymet_tile, otherwise = NA_character_)
  tile_years |> 
    ...
    ungroup() |> 
    pivot_wider(names_from = "param", names_glue = "{param}_filename", values_from = "filename")
}

extract_daymet_tile_values <- function (filename, latitude, longitude, daymet_dir = "data/daymet") {
...
extract_daymet_airtemp <- function (tmin_filename, tmax_filename, latitude, longitude) {
...
extract_daymet_tile_years <- function (combined_station_tile_years, daymet_last_year) {
