transform_output_data <- function (combined_data_airtemp, station_wbd) {
  combined_data_airtemp |> 
    ...
    rowwise() |> 
    mutate(
      data = list(select(data, date, temp_c = mean_temp_c, airtemp_c = mean_airtemp_c)),
      start = min(data$date),
      end = max(data$date),
      n = sum(!is.na(data$temp_c)),
      filename = glue("{toupper(dataset)}_{toupper(snakecase::to_snake_case(station_id))}.json")
    ) |> 
    ungroup()
}
...
export_wbd_json <- function(huc, id_field, output_gis_dir) {
  ...
  huc %>%
    ...
    st_write(filepath, append = FALSE, quiet = TRUE, layer_options = c(
